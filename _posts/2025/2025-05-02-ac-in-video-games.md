...
2. Players run their game clients on their device. Within PC games, the overwhelming majority ([over 96% at the time of writing this]((https://store.steampowered.com/hwsurvey))) run on Windows OS; different devices and operating systems (e.g. Android, Consoles, etc.) are architected differently. The steps that follow align to a Windows-based PC client.
