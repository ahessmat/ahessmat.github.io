...
This module was by far my favorite of the bunch (quite possibly because it so closely resembled my actual day-to-day). I consistently was learning new content across each of the sections, which was quite refreshing. Moreover, the Skills Assessment was phenomenal in being so open-ended in its approach; engaging it brought forward so many different ideas in trying to make it work and learning about **NodeJS** more generally. Because of how the module is structured, I ended-up putting together 3-4 exploit solutions that worked against my localized server, but only 1 that worked against the host; in the process of doing so, it felt really nice to exercise some creativity in how to go about engaging the problem, and it felt nice to apply lessons from elsewhere in iterating those approaches. For example, one of the problems the module grapples with is data extraction (suggesting either hijacking the response body, the error message body, or inferring the content via time-based blind attacks) - in the process of going about the module, I worked on applying a different method altogether: out-of-band messages, encoding responses into outbound requests (which worked locally, but failed in the exercise). Altogether, this module took longer than others and - in my opinion - was one of the most enjoyable.
